...
<html>
  <head>
	...
	function addResults(response) {  
		...
		if(response.groups.length>0) {
			for(var i=0; i< response.groups.length; i++) {
				...
				eCnt.innerHTML = response.groupsCount[response.groups[i].id] + " Groups and " + response.usersCount[response.groups[i].id] + " Users";
			...
			var eTr = document.createElement('tr');
			var eUsername = document.createElement('td');
			eUsername.innerHTML = "No results";
			eTr.appendChild(eUsername);
			eContent.appendChild(eTr);
		} 
	}
	...
	<body>
		<g:render template="/systems/systemsSearchForm" />
		<g:render template="/systems/systemsSearchResults" />
	</body>
</html>